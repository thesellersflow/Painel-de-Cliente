...
def main():
    ...
    try:
        ...
        vendas = {'trends': {}, 'premissas': {}, 'top_subindo': [], 'top_caindo': [], 'ranking_am': [], 'sensibilidade': {}, 'canal_view': {}}
...
def read_vendas(service):
    ...
    Cada seção é protegida individualmente — se uma falhar, as outras continuam."""
    ...
    # 1) Base_Historico — tendência por cliente/canal
    try:
        rows = read_sheet(service, VENDAS_ID, "'Base_Historico'!A4:J2000")
        print(f"  Base_Historico: {len(rows)} rows brutos")
        for row in rows[1:]:
            key = cell(row, 1)
            canal = cell(row, 2)
            am = cell(row, 4)
            if not key or not canal: continue
            try:
        ...
        print(f"  ⚠️  Erro Base_Historico: {e}")
    ...
    try:
        ...
        print(f"  ⚠️  Erro Premissas: {e}")

    # 3) Analise — top subindo e caindo
    try:
        rows = read_sheet(service, VENDAS_ID, "'Analise'!A4:N30")
        for row in rows[1:]:
            try:
                name_up = cell(row, 1)
                if name_up and name_up.lower() not in ('top 20 subindo', 'top 20 caindo', 'top 20 estagnados', 'resumo', 'status', 'cliente + canal'):
                    var_val = cell(row, 4)
                    try:
                        var_f = round(float(var_val) * 100, 1) if var_val else 0
                    except:
                        var_f = 0
                    top_subindo.append({
            ...
            try:
                ...
                if name_down and name_down.lower() not in ('top 20 subindo', 'top 20 caindo', 'top 20 estagnados', 'resumo', 'status', 'cliente + canal'):
                    var_val = cell(row, 14)
                    try:
                        var_f = round(float(var_val) * 100, 1) if var_val else 0
                    except:
    ...
    try:
        ...
        for row in rows[1:]:
            am_name = cell(row, 2)
            if not am_name or am_name.lower() in ('account manager', 'am'): continue
            try:
                ranking_am.append({
    ...
    try:
        ...
        for row in rows[1:]:
            key = cell(row, 1)
            if not key or key.lower() in ('cliente + canal',): continue
            sensibilidade[key] = {
                'canal': cell(row, 2),
        ...
        print(f"  ⚠️  Erro Sensibilidade: {e}")
