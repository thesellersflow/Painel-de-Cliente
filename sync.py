...
# ═══════════ ALIASES CLICKUP ═══════════
# Quando o nome da pasta no ClickUp é diferente do nome na planilha,
# adicione aqui: 'nome-no-clickup': 'slug-na-planilha'
# O 'slug-na-planilha' é o nome sem espaços/acentos em minúsculo
CLICKUP_ALIASES = {
    # ClickUp → Slug da planilha
    'dux': 'duxnutrition',
    'dux nutrition': 'duxnutrition',
    'integral': 'integralmedicabrg',
    'integralmedica': 'integralmedicabrg',
    'integralmédica': 'integralmedicabrg',
    'trio': 'triocoffee',
...
# ═══════════ MAPEAMENTO DE COLUNAS DE CANAIS ═══════════
CANAL_COLS = {
...
def match_client_slug(folder_name, valid_slugs):
    """Casa o nome da pasta do ClickUp com um slug de cliente da planilha."""
    if not folder_name:
        return None
    ...
    # 3) Match por prefixo (min 4 chars pra evitar falso match)
    if len(slug) >= 4:
...
def get_clickup_tasks_fast(valid_slugs):
    """Busca todas as tarefas em poucas chamadas via team/task."""
    ...
    for t in all_tasks:
        ...
        slug = match_client_slug(folder_name, valid_set)
        if not slug:
        ...
        # Time do ClickUp inferido pelo nome da lista
        team = 'Gestão'
        ln = list_name.upper()
    ...
    # Reportar pastas não reconhecidas
    if unmatched_folders:
        print(f"  ⚠️  {len(unmatched_folders)} pastas sem match na planilha:")
        for f in sorted(unmatched_folders):
            print(f"     - {f}")
        print(f"  💡 Adicione essas pastas em CLICKUP_ALIASES no sync.py se forem clientes válidos")

    tasks_by_team = {}
    for slug, tl in tasks_by_client.items():
        for t in tl:
            team = t.get('team', 'Gestão')
            tasks_by_team.setdefault(team, []).append({
                **t,
                'client': t.get('client_name', slug),
                'client_slug': slug
            })
...
def main():
    if not CLICKUP_TOKEN:
        raise RuntimeError("CLICKUP_TOKEN missing")
